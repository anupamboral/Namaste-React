...
// * Before we start writing test cases we have to make one more configuration  which is the jest configuration ,so the next step is writing jest configuration so how we will write a jest configuration? we will write a command which is known as `npx jest --init` so basically it will initialize the jest in our project right and it will create a new configuration file for jest, you will see a new file which is jest.config.js, first of all why is npx ? because we are just executing the package and npx is a package runner ,  basically we are initializing jest  and once you run this command it will ask you for few questions .What are these inputs the first question is 'would you like to use typescript for the configuration file?' we are not using typescript type N over there to say no,  the second question it asks you is choose the test environment that will be used for testing, be careful while you are doing this , so select the 2nd option  jsdom(browser-like) , so this js dom basically when you run test cases , what is Jsdom ?, so when you run test cases , so there is no server running , there is no browser, this test cases does not run on browser will not run on Google Chrome, so  they will need a environment to run  they will need a runtime, they will need a runtime where this testing code will be executed , these test cases will be executed , for that we use jsdom ,so if we will search for dom, it is basically what is jsdom used for, js dom is a library which parses and interacts and assembles html just like browser so it is not a browser but it is like a browser which will give you the features of browser ,but understand that it is kind of like a browser its not browser its like a browser it gives you the superpowers of browser , most important superpowers of browser and  suppose if we are testing a header components or header component will be loaded inside here(redux state) , js login.config.js dom.command to initialize jest:- npx run jeat .
