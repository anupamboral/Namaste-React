...
const RestaurantMenu = () => {
  ...
  useEffect(() => {
    const fetchMenu = async () => {
      const data = await fetch(
        `https://richie-cors-proxy.glitch.me/https://www.swiggy.com/dapi/menu/pl?page-type=REGULAR_MENU&complete-menu=true&lat=22.5743545&lng=88.3628734&restaurantId=385752&catalog_qa=undefined&submitAction=ENTER`
      );
  ...
  const { itemCards: itemList3, title: title3 } =
    resInfo?.data.cards[4]?.groupedCard?.cardGroupMap?.REGULAR?.cards[4]?.card
      ?.card;
  const { itemCards: itemList4, title: title4 } =
    resInfo?.data.cards[4]?.groupedCard?.cardGroupMap?.REGULAR?.cards[5]?.card
      ?.card;
  const { itemCards: itemList5, title: title5 } =
    resInfo?.data.cards[4]?.groupedCard?.cardGroupMap?.REGULAR?.cards[6]?.card
      ?.card;
  const { itemCards: itemList6, title: title6 } =
    resInfo?.data.cards[4]?.groupedCard?.cardGroupMap?.REGULAR?.cards[7]?.card
      ?.card;
  const { itemCards: itemList7, title: title7 } =
    resInfo?.data.cards[4]?.groupedCard?.cardGroupMap?.REGULAR?.cards[8]?.card
      ?.card;
  const { itemCards: itemList8, title: title8 } =
    resInfo?.data.cards[4]?.groupedCard?.cardGroupMap?.REGULAR?.cards[9]?.card
      ?.card;
  const { itemCards: itemList9, title: title9 } =
    resInfo?.data.cards[4]?.groupedCard?.cardGroupMap?.REGULAR?.cards[10]?.card
      ?.card;
