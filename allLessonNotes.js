...
//*and remember even we declare a state variable using const , then we can still change it's value, because even it look like normal javascript variable but actually it is not, a state variable is actually a object and we can check that using console.log(type anyStateVariableName), and it will show object, so behind the scenes we are changing some properties inside an object , and that's why we can even change the value of constant variable, because eventually a state variable is object which is returned by calling useState() hook.
